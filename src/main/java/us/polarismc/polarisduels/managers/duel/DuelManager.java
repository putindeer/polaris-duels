...
public class DuelManager {
    ...
    public void sendRequest(Player requested, Player requestor, KitType kit, int rounds) {
        ...
        if (playerRequests != null && playerRequests.stream().anyMatch(duelRequest -> duelRequest.requestorUUID().equals(requestor.getUniqueId()))) {
            plugin.utils.message(requestor, "<red>You already have a pending duel request to <aqua>" + requested.getName() + "<red>.");
            return;
        }

        requests.computeIfAbsent(requested.getUniqueId(), k -> new ArrayList<>())
                .add(new DuelRequest(requested.getUniqueId(), requestor.getUniqueId(), kit, rounds));
    ...
    /**
     * Processes a duel acceptance from the requestedUUID player.
    ...
    public void acceptDuel(Player requested, Player requestor) {
        ...
        if (playerRequests == null || playerRequests.isEmpty()) {
            plugin.utils.message(requested, "<red>You have no pending duel requests.");
            return;
        }
        ...
        if (request == null) {
            plugin.utils.message(requested, "<red>You have no duel request from <aqua>" + requestor.getName() + "<red>.");
            return;
        }

        if (!canRequestedAcceptDuel(requested, requestor)) return;

        plugin.utils.message(requested, "<green>You accepted the duel request from: <aqua>" + requestor.getName());
        plugin.utils.message(requestor, "<aqua>" + requested.getName() + " <green>accepted your duel request!");
        playerRequests.remove(request);
        createDuel(request);
    }

    /**
     * Processes a duel denial from the requestedUUID player.
    ...
    public void denyDuel(Player requested, Player requestor) {
        ...
        plugin.utils.message(requested, "<red>You denied the duel request from: <aqua>" + requestor.getName());
        plugin.utils.message(requestor, "<red>" + requested.getName() + " denied your duel request.");
    }
    ...
    public void listRequests(Player player) {
        ...
        if (playerRequests == null || playerRequests.isEmpty()) {
            plugin.utils.message(player, "<red>You have no pending duel requests.");
        } else {
            plugin.utils.message(player, "<red>You have the following pending duel requests:");
            ...
            for (DuelRequest duelRequest : playerRequests) {
                ...
                plugin.utils.message(player, "<aqua>" + requestorName
                        + " <gray>| <yellow>" + kitName
                        + " <gray>| <gold>" + rounds + " rounds");
            }
        }
    }
    ...
    private void createDuel(DuelRequest request) {
        Set<UUID> playerList = new HashSet<>(Arrays.asList(request.requestedUUID(), request.requestorUUID()));
        //TODO - fix this mess please saseskoñlk: sob.:
        GameSession session = GameSession.builder().players(playerList).gameType(GameType.DUEL_1V1).rounds(request.rounds()).kit(request.kit()).build();
        DuelTeam one = new DuelTeam(session.getScoreboard(), Collections.singleton(request.requestedUUID()), session.findNextAvailableColor());
        session.getTeams().add(one);
        DuelTeam two = new DuelTeam(session.getScoreboard(), Collections.singleton(request.requestorUUID()), session.findNextAvailableColor());
        session.getTeams().add(two);
        Optional<ArenaEntity> arena = plugin.getArenaManager().assignArena(session);
        if (arena.isEmpty()) {
            plugin.utils.message(Arrays.asList(request.requested(), request.requestor()), "<red>There are no arenas open. Try again in a bit.");
        }
    }
     ...
     * @return true if the requestorUUID can send a duel request, false otherwise
     */
    private boolean canRequestorSendDuel(Player requested, Player requestor) {
        DuelsPlayer dpRequestor = plugin.getPlayerManager().getPlayer(requestor);
        if (dpRequestor.isQueue()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request while you're in queue.");
            return false;
        }
        if (dpRequestor.isDuel() || dpRequestor.isStartingDuel()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request while you're dueling.");
            return false;
        }
        if (dpRequestor.hasParty()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request while you're in a party.");
            return false;
        }
        DuelsPlayer dpRequested = plugin.getPlayerManager().getPlayer(requested);
        if (dpRequested.isQueue()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request to someone who is in queue.");
            return false;
        }
        if (dpRequested.isDuel() || dpRequested.isStartingDuel()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request to someone who is dueling.");
            return false;
        }
        if (dpRequested.hasParty()) {
            plugin.utils.message(requestor, "<red>You can't send a duel request to someone who is in a party.");
            return false;
        }
        return true;
    }
     ...
     * @return true if the requestedUUID can accept the duel, false otherwise
     */
    private boolean canRequestedAcceptDuel(Player requested, Player requestor) {
        DuelsPlayer dpRequested = plugin.getPlayerManager().getPlayer(requested);
        if (dpRequested.isQueue()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request while you're in queue.");
            return false;
        }
        if (dpRequested.isDuel() || dpRequested.isStartingDuel()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request while you're already dueling.");
            return false;
        }
        if (dpRequested.hasParty()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request while you're in a party.");
            return false;
        }
        DuelsPlayer dpRequestor = plugin.getPlayerManager().getPlayer(requestor);
        if (dpRequestor.isQueue()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request from someone who is in queue.");
            return false;
        }
        if (dpRequestor.isDuel() || dpRequestor.isStartingDuel()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request from someone who is currently dueling.");
            return false;
        }
        if (dpRequestor.hasParty()) {
            plugin.utils.message(requested, "<red>You can't accept a duel request from someone who is in a party.");
            return false;
        }
        return true;
    }
    ...
    public DuelRequest getRequest(Player requested, Player requestor) {
        ...
        if (playerRequests != null) {
            return playerRequests.stream()
                    .filter(duelRequest -> duelRequest.requestorUUID().equals(requestor.getUniqueId()))
                    .findFirst()
                    .orElse(null);
        }
        return null;
