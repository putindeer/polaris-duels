...
public class DuelManager {
    ...
    private void createDuel(DuelRequest request) {
        ...
        if (arena.isEmpty()) {
            plugin.utils.message(Arrays.asList(request.requested(), request.requestor()), "&cThere are no arenas open. Try again in a bit.");
        }
    }
    ...
    private boolean canRequestorSendDuel(Player requested, Player requestor) {
        ...
        if (dpRequestor.hasParty()) {
            plugin.utils.message(requestor, "&cYou can't send a duel request while you're in a party.");
            return false;
        }
        ...
        if (dpRequested.hasParty()) {
            plugin.utils.message(requestor, "&cYou can't send a duel request to someone who is in a party.");
            return false;
        }
        return true;
    }
    ...
    private boolean canRequestedAcceptDuel(Player requested, Player requestor) {
        ...
        if (dpRequested.hasParty()) {
            plugin.utils.message(requested, "&cYou can't accept a duel request while you're in a party.");
            return false;
        }
        DuelsPlayer dpRequestor = plugin.getPlayerManager().getPlayer(requestor);
        if (dpRequestor.isQueue()) {
        ...
        if (dpRequestor.hasParty()) {
            plugin.utils.message(requested, "&cYou can't accept a duel request from someone who is in a party.");
            return false;
        }
        return true;
    }
